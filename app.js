...
function deleteHistPeriod() {
  ...
  if (!confirm(`¿Eliminar "${mp.label || "este mes"}" del histórico? Se borrarán también todos sus gastos, ingresos y ahorros. Esto no se puede deshacer.`)) return;
  state.categories.forEach(cat => { cat.transactions = (cat.transactions || []).filter(t => t.periodId !== mp.id); });
  state.incomeCategories.forEach(cat => { cat.entries = (cat.entries || []).filter(e => e.periodId !== mp.id); });
  state.savings = state.savings.filter(s => s.periodId !== mp.id);
