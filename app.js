...
function ensureShape() {
  ...
  const hasClosedPeriods = state.periods.some(p => p.endDate);
  if (!open) {
    state.periods.push({ id: uid("per"), label: "Mes actual", startDate: earliest || todayISO(), endDate: null });
  } else if (!hasClosedPeriods && earliest && earliest < open.startDate) {
...
function renderSavingsList() {
  ...
  const items = state.savings.filter(s => inPeriod(s.date, mp) && countsYet(s.date, mp)).slice().reverse();
...
function deleteHistPeriod() {
  ...
  if (!confirm(`¿Eliminar "${mp.label || "este mes"}" del histórico? Los gastos e ingresos no se borran, solo dejan de estar archivados bajo este nombre.`)) return;
...
function renderHistorico() {
  ...
  const savingsThisMonth = state.savings.filter(s => inPeriod(s.date, mp)).slice().reverse();
