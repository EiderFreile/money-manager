...
function confirmCloseMonth() {
  ...
  generateRecurringForNewPeriod(newPeriod);
  closeModal("modal-close-month");
  renderGastos();
}

function generateRecurringForNewPeriod(newPeriod) {
  const today = new Date();
  state.categories.forEach(cat => {
    (cat.transactions || []).filter(t => t.recurring && t.periodId !== newPeriod.id).forEach(t => {
  ...
  state.incomeCategories.forEach(cat => {
    (cat.entries || []).filter(e => e.recurring && e.periodId !== newPeriod.id).forEach(e => {
