...
try {
  if (typeof db !== "undefined") {
    useFirebase = true;
    db.on("value", snap => {
      // Siempre se procesa lo que llega de Firebase como la versión buena,
      // incluido el eco de nuestro propio guardado (evita condiciones de carrera
      // que podían dejar abiertos dos periodos "actuales" a la vez).
      const data = snap.val();
...
function ensureShape() {
  ...
  // Asegurar que existe un periodo abierto, con número de secuencia
  let openPeriods = state.periods.filter(p => !p.endDate);
  let open;
  if (openPeriods.length === 0) {
    open = { id: uid("per"), seq: 1, label: "Mes actual", startDate: todayISO(), endDate: null };
    state.periods.push(open);
  } else if (openPeriods.length === 1) {
    open = openPeriods[0];
  } else {
    // Se colaron dos (o más) periodos abiertos a la vez (por una carrera al sincronizar).
    // Nos quedamos con el de mayor seq (el más "nuevo"); a los demás les reasignamos
    // todo lo que tuvieran etiquetado y los eliminamos.
    openPeriods.sort((a, b) => (b.seq || 0) - (a.seq || 0));
    open = openPeriods[0];
    const duplicateIds = openPeriods.slice(1).map(p => p.id);
    state.categories.forEach(c => (c.transactions || []).forEach(t => { if (duplicateIds.includes(t.periodId)) t.periodId = open.id; }));
    state.incomeCategories.forEach(c => (c.entries || []).forEach(e => { if (duplicateIds.includes(e.periodId)) e.periodId = open.id; }));
    state.savings.forEach(s => { if (duplicateIds.includes(s.periodId)) s.periodId = open.id; });
    state.periods = state.periods.filter(p => !duplicateIds.includes(p.id));
  }
  if (open.seq === undefined) {
    const maxSeq = state.periods.reduce((m, p) => Math.max(m, p.seq || 0), 0);
    open.seq = maxSeq + 1;
  }
  state.periods.forEach((p, i) => { if (p.seq === undefined) p.seq = i + 1; });

  // Migración de una sola vez: cualquier gasto/ingreso/ahorro sin periodId todavía
  // (de antes de este cambio) se etiqueta con el periodo abierto actual
  state.categories.forEach(c => (c.transactions || []).forEach(t => { if (!t.periodId) t.periodId = open.id; }));
  state.incomeCategories.forEach(c => (c.entries || []).forEach(e => { if (!e.periodId) e.periodId = open.id; }));
  state.savings.forEach(s => { if (!s.periodId) s.periodId = open.id; });

  // Limpieza de recurrentes duplicados: si dentro del mismo periodo hay más de una
  // copia del mismo recurrente (misma descripción, importe y día), nos quedamos con
  // una sola y borramos el resto (restos de cierres de mes repetidos por el bug de sync).
  state.categories.forEach(c => {
    const seen = new Set();
    c.transactions = (c.transactions || []).filter(t => {
      if (!t.recurring) return true;
      const key = t.periodId + "|" + t.desc + "|" + t.amount + "|" + (t.recDay || "");
  ...
  state.incomeCategories.forEach(c => {
    ...
    c.entries = (c.entries || []).filter(e => {
      ...
      const key = e.periodId + "|" + e.desc + "|" + e.amount + "|" + (e.recDay || "");
      if (seen.has(key)) return false;
      seen.add(key);
      return true;
    });
...
function sortedPeriods() { return state.periods.slice().sort((a, b) => (a.seq || 0) - (b.seq || 0)); }
function currentPeriod() { return state.periods.find(p => !p.endDate) || sortedPeriods()[sortedPeriods().length - 1]; }
function belongsToPeriod(item, period) { return period && item.periodId === period.id; }
...
/* Decide a qué periodo se etiqueta un movimiento nuevo, según su fecha */
function resolvePeriodId(dateStr) {
  if (!dateStr || dateStr <= todayISO()) return currentPeriod().id;
  return "next"; // fecha futura: se etiqueta como "próximo mes" hasta que se cierre el actual
}

/* ===== HELPERS DE CATEGORÍA (por periodo) ===== */
function catSpentInMonth(cat, period) {
  return (cat.transactions || []).filter(t => t.amount < 0 && belongsToPeriod(t, period)).reduce((s, t) => s + Math.abs(t.amount), 0);
}
function catIngresosInMonth(cat, period) {
  return (cat.transactions || []).filter(t => t.amount > 0 && belongsToPeriod(t, period)).reduce((s, t) => s + t.amount, 0);
}
...
function incCatTotalInMonth(ic, period) { return (ic.entries || []).filter(e => belongsToPeriod(e, period)).reduce((s, e) => s + e.amount, 0); }
function savingsInMonth(period) { return state.savings.filter(s => belongsToPeriod(s, period)).reduce((s2, s) => s2 + s.amount, 0); }
...
function renderMovements() {
  const mp = currentPeriod();
  const items = [];
  state.categories.forEach(c => (c.transactions || []).forEach(t => { if (t.periodId === mp.id || t.periodId === "next") items.push({ desc: t.desc, amount: t.amount, date: t.date, tag: c.name }); }));
  state.incomeCategories.forEach(c => (c.entries || []).forEach(e => { if (e.periodId === mp.id || e.periodId === "next") items.push({ desc: e.desc, amount: e.amount, date: e.date, tag: c.name }); }));
  state.savings.forEach(s => { if (s.periodId === mp.id) items.push({ desc: s.desc, amount: s.amount, date: s.date, tag: "Ahorro" }); });
...
function renderCatDetailInline(c, mp, balance) {
  const txs = (c.transactions || []).filter(t => t.periodId === mp.id).slice().reverse();
  const nextTxs = (c.transactions || []).filter(t => t.periodId === "next").slice().reverse();
  const rowHtml = t => `
    <div class="movement-row">
      <span>${t.desc}${t.periodId === "next" ? " <em style=\"opacity:.7;\">(próximo mes)</em>" : ""}</span>
  ...
  const txHtml = (txs.length || nextTxs.length) ? txs.map(rowHtml).join("") + nextTxs.map(rowHtml).join("") : `<p class="empty-hint">Sin movimientos este mes</p>`;
...
function renderIncomeCategoriesList() {
  ...
  list.innerHTML = state.incomeCategories.length ? state.incomeCategories.map(c => {
    ...
    const entries = (c.entries || []).filter(e => e.periodId === mp.id).slice().reverse();
    const nextEntries = (c.entries || []).filter(e => e.periodId === "next").slice().reverse();
    const entryRow = e => `
      <div class="movement-row">
        <span>${e.desc}${e.periodId === "next" ? " <em style=\"opacity:.7;\">(próximo mes)</em>" : ""}</span>
        <span style="display:flex; align-items:center; gap:8px;">
    ...
    const entriesHtml = (entries.length || nextEntries.length) ? entries.map(entryRow).join("") + nextEntries.map(entryRow).join("") : `<p class="empty-hint">Sin ingresos este mes</p>`;
    return `<div class="cat-card">
      <div class="cat-head">
...
function renderSavingsList() {
  ...
  const items = state.savings.filter(s => s.periodId === mp.id).slice().reverse();
  list.innerHTML = items.length ? `<div class="card">` + items.map(s => `
    ...
    </div>`).join("") + `</div>` : `<p class="empty-hint">Todavía no hay ahorros este mes</p>`;
}
...
function confirmCloseMonth() {
  ...
  p.label = label;
  p.endDate = todayISO();
  const newPeriod = { id: uid("per"), seq: (p.seq || 0) + 1, label: "Mes actual", startDate: todayISO(), endDate: null };
  state.periods.push(newPeriod);

  // Todo lo que estaba etiquetado "next" (fecha futura, del mes que viene) pasa a ser del nuevo periodo real
  state.categories.forEach(cat => (cat.transactions || []).forEach(t => { if (t.periodId === "next") t.periodId = newPeriod.id; }));
  state.incomeCategories.forEach(cat => (cat.entries || []).forEach(e => { if (e.periodId === "next") e.periodId = newPeriod.id; }));
  state.savings.forEach(s => { if (s.periodId === "next") s.periodId = newPeriod.id; });

  generateRecurringForNewPeriod(p, newPeriod);
  closeModal("modal-close-month");
  renderGastos();
}

function generateRecurringForNewPeriod(closingPeriod, newPeriod) {
  const today = new Date();
  state.categories.forEach(cat => {
    // Solo los recurrentes que pertenecían al periodo que se acaba de cerrar
    // (nunca de todo el histórico, para no resucitar recurrentes de meses ya pasados)
    const already = new Set((cat.transactions || []).filter(t => t.periodId === newPeriod.id && t.recurring).map(t => t.desc + "|" + t.amount + "|" + (t.recDay || "")));
    (cat.transactions || []).filter(t => t.recurring && t.periodId === closingPeriod.id).forEach(t => {
      if (t.recEnd && t.recEnd < newPeriod.startDate) return; // ya venció
      const key = t.desc + "|" + t.amount + "|" + (t.recDay || "");
      if (already.has(key)) return; // ya existe una copia en el periodo nuevo, no duplicar
      already.add(key);
      ...
      cat.transactions.push({ id: uid("tx"), desc: t.desc, amount: t.amount, date: d.toISOString(), periodId: newPeriod.id, recurring: true, recDay: t.recDay, recNoEnd: t.recNoEnd, recEnd: t.recEnd });
    });
  });
  state.incomeCategories.forEach(cat => {
    const already = new Set((cat.entries || []).filter(e => e.periodId === newPeriod.id && e.recurring).map(e => e.desc + "|" + e.amount + "|" + (e.recDay || "")));
    (cat.entries || []).filter(e => e.recurring && e.periodId === closingPeriod.id).forEach(e => {
      if (e.recEnd && e.recEnd < newPeriod.startDate) return;
      const key = e.desc + "|" + e.amount + "|" + (e.recDay || "");
      if (already.has(key)) return;
      already.add(key);
      const day = e.recDay && e.recDay >= 1 && e.recDay <= 28 ? e.recDay : today.getDate();
      const d = new Date(today.getFullYear(), today.getMonth(), day, 12, 0, 0);
      cat.entries.push({ id: uid("inc"), desc: e.desc, amount: e.amount, date: d.toISOString(), periodId: newPeriod.id, recurring: true, recDay: e.recDay, recNoEnd: e.recNoEnd, recEnd: e.recEnd });
    });
  });
}
...
function saveExpense() {
  ...
  const periodId = resolvePeriodId(date);
  cat.transactions = cat.transactions || [];
  if (editingExpenseTxId) {
    const tx = cat.transactions.find(t => t.id === editingExpenseTxId);
    if (tx) { tx.desc = desc; tx.amount = -amount; tx.date = date; tx.periodId = periodId; Object.assign(tx, rec); }
  } else {
    cat.transactions.push({ id: uid("tx"), desc, amount: -amount, date, periodId, ...rec });
  }
  editingExpenseTxId = null;
...
function saveCategoryIncome() {
  ...
  if (editingCatIncomeTxId) {
    ...
    if (tx) { tx.desc = desc; tx.amount = amount; tx.date = date; tx.periodId = periodId; }
  } else {
    cat.transactions.push({ id: uid("tx"), desc, amount, date, periodId });
  }
  editingCatIncomeTxId = null;
...
function saveIncome() {
  ...
  if (editingIncomeEntryId) {
    ...
    if (e) { e.desc = desc; e.amount = amount; e.date = date; e.periodId = periodId; Object.assign(e, rec); }
  } else {
    cat.entries.push({ id: uid("inc"), desc, amount, date, periodId, ...rec });
  }
  editingIncomeEntryId = null;
...
function saveSaving() {
  ...
  if (editingSavingId) {
    ...
    state.savings.push({ id: uid("sav"), desc, amount, date: todayISO(), periodId: currentPeriod().id });
  }
  editingSavingId = null;
...
function deleteHistPeriod() {
  const mp = histPeriod();
  if (!mp) return;
  if (!mp.endDate) { alert("El mes actual (todavía abierto) no se puede eliminar así."); return; }
  if (!confirm(`¿Eliminar "${mp.label || "este mes"}" del histórico? Los gastos e ingresos no se borran, pasan a contar en el mes actual.`)) return;
  const cp = currentPeriod();
  state.categories.forEach(cat => (cat.transactions || []).forEach(t => { if (t.periodId === mp.id) t.periodId = cp.id; }));
  state.incomeCategories.forEach(cat => (cat.entries || []).forEach(e => { if (e.periodId === mp.id) e.periodId = cp.id; }));
  state.savings.forEach(s => { if (s.periodId === mp.id) s.periodId = cp.id; });
  state.periods = state.periods.filter(p => p.id !== mp.id);
  histIndex = null;
  persist();
  renderHistorico();
}
...
function renderHistorico() {
  ...
  const savingsThisMonth = state.savings.filter(s => s.periodId === mp.id).slice().reverse();
  const savingsTotal = savingsThisMonth.reduce((s, x) => s + x.amount, 0);
  const savingsRows = savingsThisMonth.length
    ? savingsThisMonth.map(s => `<div class="movement-row"><span>${s.desc}</span><span style="color:var(--teal-soft-text-2);">+${fmt(s.amount)}</span></div>`).join("")
    : `<p class="empty-hint">Sin ahorros este mes</p>`;
  ...
  document.getElementById("hist-content").innerHTML = `
    ...
    <div class="card"><p class="section-label">Ahorros del mes <span style="float:right; color:var(--teal-soft-text-2);">${fmt(savingsTotal)}</span></p>${savingsRows}</div>
  `;
}
...
function saveManualMonth() {
  ...
  const maxSeq = state.periods.reduce((m, p) => Math.max(m, p.seq || 0), 0);
  const newPeriod = { id: uid("per"), seq: maxSeq + 1, label, startDate, endDate };
  state.periods.push(newPeriod);
  ...
  document.querySelectorAll(".mm-gasto").forEach(inp => {
    ...
    if (cat) { cat.transactions = cat.transactions || []; cat.transactions.push({ id: uid("tx"), desc: label, amount: -val, date: midDate, periodId: newPeriod.id }); }
  });
  document.querySelectorAll(".mm-ingreso").forEach(inp => {
    const val = parseFloat(inp.value);
    if (!val) return;
    const cat = state.incomeCategories.find(c => c.id === inp.dataset.id);
    if (cat) { cat.entries = cat.entries || []; cat.entries.push({ id: uid("inc"), desc: label, amount: val, date: midDate, periodId: newPeriod.id }); }
  });
  const savingsVal = parseFloat(document.getElementById("manual-month-savings").value);
  if (savingsVal) state.savings.push({ id: uid("sav"), desc: label, amount: savingsVal, date: midDate, periodId: newPeriod.id });
