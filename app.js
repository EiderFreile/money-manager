...
function catSpentInMonth(cat, mp) {
  return (cat.transactions || []).filter(t => t.amount < 0 && monthPrefix(t.date) === mp).reduce((s, t) => s + Math.abs(t.amount), 0);
}
function catIngresosInMonth(cat, mp) {
  return (cat.transactions || []).filter(t => t.amount > 0 && monthPrefix(t.date) === mp).reduce((s, t) => s + t.amount, 0);
}
function catGastadoNeto(cat, mp) { return Math.max(0, catSpentInMonth(cat, mp) - catIngresosInMonth(cat, mp)); }
function catBalanceReal(cat, mp) { return catSpentInMonth(cat, mp) - catIngresosInMonth(cat, mp); }
function incCatTotalInMonth(ic, mp) { return (ic.entries || []).filter(e => monthPrefix(e.date) === mp).reduce((s, e) => s + e.amount, 0); }

function allMonthsWithData() {
  const set = new Set();
  state.categories.forEach(c => (c.transactions || []).forEach(t => set.add(monthPrefix(t.date))));
  state.incomeCategories.forEach(c => (c.entries || []).forEach(e => set.add(monthPrefix(e.date))));
...
function renderCatDetailInline(c, mp, balance) {
  const txs = (c.transactions || []).filter(t => monthPrefix(t.date) === mp).slice().reverse().slice(0, 5);
  ...
  return `<div class="cat-detail-inline" onclick="event.stopPropagation()">
    ${c.transactions.some(t => t.amount > 0) ? `<p class="remaining-text">Balance real (con ingresos): <strong>${fmt(balance)}</strong></p>` : ""}
    ${txHtml}
  </div>`;
}
...
function renderIncomeCategoriesList() {
  ...
  list.innerHTML = state.incomeCategories.length ? state.incomeCategories.map(c => {
    ...
    const entries = (c.entries || []).filter(e => monthPrefix(e.date) === mp).slice().reverse().slice(0, 5);
...
/* ===== ANALYTICS ===== */
function renderAnalytics() { renderAnalyticsFor(currentMonthPrefix(), new Date().getFullYear().toString()); }
...
function renderAnalyticsFor(mp, year) {
  ...
  const yearMonths = allMonthsWithData().filter(m => m.startsWith(year));
  const gastosPorMes = yearMonths.map(m => state.categories.reduce((s, c) => s + catGastadoNeto(c, m), 0));
  const ingresosPorMes = yearMonths.map(m => state.incomeCategories.reduce((s, c) => s + incCatTotalInMonth(c, m), 0));
  const nMeses = yearMonths.length || 1;
  ...
  gastoTable.innerHTML = "<tr><th>Categoría</th><th>Prom. Mes</th><th>Total Año</th></tr>" +
    (state.categories.length ? state.categories.map(c => {
      const monthsWith = yearMonths.filter(m => catSpentInMonth(c, m) > 0 || catIngresosInMonth(c, m) > 0);
      const total = monthsWith.reduce((s, m) => s + catGastadoNeto(c, m), 0);
      const avg = total / (monthsWith.length || 1);
  ...
  incTable.innerHTML = "<tr><th>Categoría</th><th>Prom. Mes</th><th>Total Año</th></tr>" +
    (state.incomeCategories.length ? state.incomeCategories.map(c => {
      const monthsWith = yearMonths.filter(m => incCatTotalInMonth(c, m) > 0);
      const total = monthsWith.reduce((s, m) => s + incCatTotalInMonth(c, m), 0);
      const avg = total / (monthsWith.length || 1);
      return `<tr><td>${c.name}</td><td>${fmt(avg)}</td><td class="col-year">${fmt(total)}</td></tr>`;
    }).join("") : `<tr><td colspan="3" class="empty-hint">Sin categorías</td></tr>`);
...
let histMonth = new Date().getMonth() + 1; // 1-12

function changeHistYear(delta) { histYear += delta; renderHistorico(); }
...
function renderHistorico() {
  document.getElementById("hist-year-label").textContent = histYear;
  document.getElementById("hist-month-label").textContent = MESES[histMonth - 1];
  const mp = histYear + "-" + String(histMonth).padStart(2, "0");
  const year = String(histYear);
  ...
  document.getElementById("hist-content").innerHTML = `
    <div class="metric-grid-3" style="margin-bottom:14px;">
      ...
      <div class="metric"><p class="label">Balance Ingresos − Gastos</p><p class="value">${fmt(ingresosMes - gastosMes)}</p></div>
    </div>
...
/* Añadir mes pasado: mete un total (suma) por categoría, fechado el día 15 de ese mes */
...
function saveManualMonth() {
  const monthVal = document.getElementById("manual-month-value").value; // "YYYY-MM"
  if (!monthVal) return;
  const dateStr = monthVal + "-15T00:00:00.000Z";

  document.querySelectorAll(".mm-gasto").forEach(inp => {
    const val = parseFloat(inp.value);
    if (!val) return;
    const cat = state.categories.find(c => c.id === inp.dataset.id);
    if (cat) { cat.transactions = cat.transactions || []; cat.transactions.push({ id: uid("tx"), desc: "Mes " + monthVal, amount: -val, date: dateStr }); }
  });
  document.querySelectorAll(".mm-ingreso").forEach(inp => {
    const val = parseFloat(inp.value);
    if (!val) return;
    const cat = state.incomeCategories.find(c => c.id === inp.dataset.id);
    if (cat) { cat.entries = cat.entries || []; cat.entries.push({ id: uid("inc"), desc: "Mes " + monthVal, amount: val, date: dateStr }); }
  });
  const savingsVal = parseFloat(document.getElementById("manual-month-savings").value);
  if (savingsVal) state.savings.push({ id: uid("sav"), desc: "Mes " + monthVal, amount: savingsVal, date: dateStr });
