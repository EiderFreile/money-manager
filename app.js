  ...
  if (acc.type === "remunerada") {
    ...
    catList.innerHTML = cats.length ? cats.map((c, i) => {
      ...
      return `<div class="cat-card ${tone}">
        ...
        <div class="progress-track" onclick="toggleCatExpand('${c.id}')" style="cursor:pointer;"><div class="progress-fill ${c.spent > c.budget && c.budget > 0 ? "over" : ""}" style="width:${pct}%;"></div></div>
...
function renderCatDetailInline(c) {
  const warn = (c.budget > 0 && c.spent > c.budget)
    ? `<p class="warn-text" style="margin-top:8px;">⚠ Pasado en ${fmt(c.spent - c.budget)}</p>` : "";
  ...
  return `
    <div class="cat-detail-inline" onclick="event.stopPropagation()">
      ...
      <div class="btn-row">
        ...
        <button class="btn" style="background:#E1F5EE; border-color:#BEE9DA; color:var(--teal-soft-text);" onclick="openIncomeModal()">Ingreso</button>
      </div>
    ...
    return `<div class="cat-card ${tone}" style="margin-bottom:6px; padding:8px 10px;">
      <div class="cat-head" style="margin:0;">
        ...
        <span style="display:flex; align-items:center; gap:8px;">
          ...
          <button class="icon-btn danger" onclick="deleteIncomeCategory('${c.id}')">🗑</button>
        </span>
      </div>
    </div>`;
  }).join("") : "";
...
function saveIncome() {
  ...
  state.incomeList[acc].push({ desc, amount, who, type, categoryId });
...
function openAddExistingIncomeCategoryModal() {
  ...
  const available = state.incomeCategoryDefs.filter(d => !usedIds.includes(d.id));
  ...
  wrap.innerHTML = available.length
    ? available.map(d => `
      <label class="field-checkbox" style="margin-bottom:8px;">
        <input type="checkbox" class="existing-inccat-check" value="${d.id}">
        <span style="margin:0;">${d.emoji ? d.emoji + " " : ""}${d.name}</span>
      </label>`).join("")
    : `<p class="empty-hint">No tienes más categorías de ingreso creadas en otras cuentas</p>`;
  openModal("modal-add-existing-income-category");
}
function saveAddExistingIncomeCategories() {
  const acc = state.currentAccount;
  const checked = Array.from(document.querySelectorAll(".existing-inccat-check:checked")).map(el => el.value);
...
function openAddExistingCategoryModal() {
  ...
  const available = state.categoryDefs.filter(d => !usedIds.includes(d.id));
  const wrap = document.getElementById("existing-cat-list");
  wrap.innerHTML = available.length
    ? available.map(d => `
      <label class="field-checkbox" style="margin-bottom:8px;">
        <input type="checkbox" class="existing-cat-check" value="${d.id}">
        <span style="margin:0;">${d.emoji ? d.emoji + " " : ""}${d.name}</span>
      </label>`).join("")
    : `<p class="empty-hint">No tienes más categorías creadas en otras cuentas</p>`;
  openModal("modal-add-existing-category");
}
function saveAddExistingCategories() {
  const acc = state.currentAccount;
  const checked = Array.from(document.querySelectorAll(".existing-cat-check:checked")).map(el => el.value);
  checked.forEach(defId => {
...
function renderAnalytics() {
  ...
  const pieMap = {};
  catsForPie.forEach(c => { pieMap[c.name] = (pieMap[c.name] || 0) + c.spent; });
  ...
  const liveSpent = catsForPie.reduce((s, c) => s + c.spent, 0);
