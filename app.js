...
function persist() {
  if (!dataLoaded) { console.warn("[DEBUG persist] bloqueado: dataLoaded todavía es false"); return; }
  localStorage.setItem("moneyManagerState", JSON.stringify(state));
  if (useFirebase) {
    try {
      db.set(state).then(() => console.log("[DEBUG persist] guardado en Firebase OK"))
        .catch(err => console.error("[DEBUG persist] Firebase rechazó el guardado:", err));
    } catch (e) { console.error("[DEBUG persist] excepción al guardar:", e); }
  }
}
...
function saveExpense() {
  ...
  console.log("[DEBUG saveExpense]", { acc, catId, catFound: !!cat, editingTxId, amountRaw: document.getElementById("expense-amount").value });
  if (!cat) { console.warn("[DEBUG saveExpense] cat no encontrada, se aborta el guardado"); return; }
  const desc = document.getElementById("expense-desc").value || cat.name;
  const amount = parseFloat(document.getElementById("expense-amount").value) || 0;
  if (!amount) { console.warn("[DEBUG saveExpense] amount es 0, se aborta el guardado"); return; }
  const whoGroup = document.getElementById("expense-who-field").querySelector(".tag-row");
...
function getMonthlyBalance(accId) {
  ...
  monthMap[liveMonth].spent += (state.categories[accId] || []).reduce((s, c) => s + c.spent, 0);
  return monthMap;
}
